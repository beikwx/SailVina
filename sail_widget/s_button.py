...
class SButton(object):
    ...
    def _bind_open_dir(self, event):
        self.initial_dir = self.entry_text.get()
        dir_name = SFile().open_dir(initial_dir=self.initial_dir, title=self.title, parent=self.parent)
        self.entry_text.set(dir_name)

    def bind_open_dir(self, entry_text: str, title: str, parent=""):
        """
        给按钮绑定选择一个目录
        :param entry_text: 打开后初始位置，通常为某个entry.textvariable
        :param title: 标题
        :param parent: 在哪个控件上面显示，默认主窗口上
        """
        self.entry_text = entry_text
    ...
    def _bind_open_files(self, event):
        ...
        self.filename_text = SFile.open_files(initial_dir=self.initial_dir, title=self.title,
                                              file_type=file_type, parent=self.parent)
        self.entry_text.set(self.filename_text)

    def bind_open_files(self, entry_text, title, file_type, parent=""):
        self.entry_text = entry_text
        self.title = title
        self.file_type = file_type
        self.parent = parent
